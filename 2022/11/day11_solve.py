...
class Monkey():
    ...
    def process_item(self, monkeys):
        ...
        self.debug("Test ",self.test_desc, ' is ', self.test(item), ' item with level ',item,' is thrown to ',target )        
...
def setup_puzzle(puzzle_text, constructor=Monkey):
    ...
    for line in map(str.strip, puzzle_text.split('\n')):
        if line.startswith('Monkey'):
            ...
            monkeys[number].items = list(map(int, filter(lambda x: len(x.strip())>0, line.split(':')[-1].split(', '))))
...
print(solve1(sample))
print(solve1(read_input('11')))
# print(solve1(puzzle))
print(solve2(sample))
print(solve2(read_input('11')))
